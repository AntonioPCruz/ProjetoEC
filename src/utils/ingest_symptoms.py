...
def ingest_data():
    ...
    for _, row in df_desc.iterrows():
        cur.execute(
            "INSERT INTO diseases (name, description) VALUES (%s, %s) ON CONFLICT (name) DO NOTHING",
    ...
    for _, row in df_sev.iterrows():
        cur.execute(
            "INSERT INTO symptoms (name, severity_weight) VALUES (%s, %s) ON CONFLICT (name) DO NOTHING",
    ...
    for _, row in df_prec.iterrows():
        ...
        if res:
            ...
            for i in range(1, 5):
                ...
                if pd.notna(prec):
                    cur.execute("INSERT INTO disease_precautions (disease_id, precaution) VALUES (%s, %s)", (d_id, prec))
